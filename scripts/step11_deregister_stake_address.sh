...
SYNC=$(cardano-cli query tip $NETWORK_MAGIC | jq '.syncProgress')
...
#cd $UTXO_KEYS_PATH
cardano-cli query utxo --address $(cat $UTXO_KEYS_PATH/payment.addr) $NETWORK_MAGIC > $TXS_PATH/fullUtxo5.out
...
stakePoolRewards=$(cardano-cli query stake-address-info $NETWORK_MAGIC --address $(cat $UTXO_KEYS_PATH/stake.addr) | jq -r '.[0].rewardAccountBalance')
echo stakePoolRewards: $stakePoolRewards

currentSlot=$(cardano-cli query tip $NETWORK_MAGIC | jq -r '.slot')
echo Current Slot: $currentSlot

keyDeposit=$(cardano-cli query protocol-parameters   --cardano-mode  $NETWORK_MAGIC | jq '.stakeAddressDeposit')
...
cardano-cli transaction submit --tx-file $TXS_PATH/tx_faucet.signed $NETWORK_MAGIC
