...
CURRENT_EPOCH=$(cardano-cli query tip $NETWORK_MAGIC | jq '.epoch')
...
#cd $UTXO_KEYS_PATH
cardano-cli query utxo --address $(cat $UTXO_KEYS_PATH/payment.addr) $NETWORK_MAGIC > $TXS_PATH/fullUtxo4.out
...
cardano-cli transaction submit --tx-file $TXS_PATH/tx4.signed $NETWORK_MAGIC
